...
void ScalarConverter::convert(const std::string &literal)
{
    ...
    else if(checkDigit(literal))
    {
        ...
        if(std::isprint(static_cast<char>(nbr)) && static_cast<int>(nbr) > 32 && static_cast<int>(nbr) < 127) // check >= 32 if i add it ??
            std::cout << "char: " << static_cast<char>(nbr) <<std::endl;
        else
            std::cout << "char: Non displayable" << std::endl;
        std::cout << "float: " << std::fixed << std::setprecision(2) << static_cast<float>(nbr)  <<std::endl;
